...
public class ConfigTONew {
	...
	public static ConfigTONew load(File file) throws ConfigException {
		try {
			return new Persister().read(ConfigTONew.class, file);
		}
		catch (Exception ex) {
	...
	public org.syncany.config.to.ConnectionTO getConnectionTO() {
		return connectionTO;
	}
