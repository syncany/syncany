...
public class StartDialog extends DefaultWizardPanel {
	 ...
	 * @param style
	 */
	public StartDialog(Shell parent, int style) {
		super(WizardType.NEXT, parent, style);
	}
	...
	protected Composite createComposite(Shell shell){
		...
		createStorageRadio.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		...
		connectStorageRadio.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
	...
	protected void handleAction(WizardAction action) {
		if (action == WizardAction.NEXT){
			if (createStorageRadio.getSelection()){
				...
				this.shell.dispose();
				ConnectDialog sd = new ConnectDialog(getParent(), SWT.APPLICATION_MODAL);
				sd.open();
			}
		}
	}
